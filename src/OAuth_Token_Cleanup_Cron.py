...
parser.add_argument('--email', type=str, help='comma-separated email addresses to notify of token cleanup', default=None)
parser.add_argument('--smtpserver', type=str, help='SMTP relay server to use to send notification email', default=None)
parser.add_argument('--sender', type=str, help='email address to use as the sender', default=None)
parser.add_argument('--email-title', type=str, help='email title template', default="{dbhost} OAuth Token Cleanup executed at {exec_time}")
...
def sendEmailAlert(BODY, dbhost, subject_template, exec_time):
    if args.email and args.smtpserver:
        subject = subject_template.format(dbhost=dbhost, exec_time=exec_time)
